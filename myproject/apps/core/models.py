...
class MetaTagsBase(models.Model):
    meta_keywords = models.CharField(
        ...
        max_length = 255,
        blank = True,
        help_text = _("Seperate keywords with commas."),
    )
...
class CreationModificationDateBase(models.Model):
    '''
    Abstract base creationwith a creation and modification date and time
    '''
...
class UrlBase(models.Model):
    """
    A replacement for get_absolute_url() Models extending this maxin should have either get_url get_url_path implemented
    """
    # Meta a class container with some option(metadata) attached to the model
    class Meta:
        abstract =True
    def get_url(self):
        if hasattr(self.get_url_path, "don't_recurse"):
    ...
    def get_url_path(self):
        if hasattr(self.get_url, "don't_recurse"):
            raise NotImplementedError
